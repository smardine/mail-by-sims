...
public class MlCompteMail {
	...
	private void initialiseCompte(int p_idCompte) {
		...
		ArrayList<String> defCompte = accesCompte.getCompteByID(p_idCompte);
	 ...
	 * @param bd
	 */
	private void obtainIdDossierPrincipaux() {
		EnDossierBase[] lstDossierBase = EnDossierBase.values();
		for (EnDossierBase unDossier : lstDossierBase) {
			switch (unDossier) {
				case RECEPTION:
					setIdInbox(accesDossier.getIdDossier(unDossier.getLib(),
							idCompte));
					break;
				case BROUILLON:
					setIdBrouillons(accesDossier.getIdDossier(unDossier
							.getLib(), idCompte));
					break;
				case SPAM:
					setIdSpam(accesDossier.getIdDossier(unDossier.getLib(),
							idCompte));
					break;
				case CORBEILLE:
					setIdCorbeille(accesDossier.getIdDossier(
							unDossier.getLib(), idCompte));
					break;
				case ENVOYES:
					setIdEnvoye(accesDossier.getIdDossier(unDossier.getLib(),
							idCompte));
					break;
			}
		}
