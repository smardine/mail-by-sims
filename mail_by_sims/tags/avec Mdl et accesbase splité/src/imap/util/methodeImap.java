...
public final class methodeImap {
	...
	public static void miseAJourMessage(Properties props, int pIdCompte,
		...
		for (MlDossier dossier : lst) {
			...
			try {
				...
				int messBaseCount = accesMail.getnbMessageParDossier(pIdCompte,
						idDossier);
				...
				for (MlMessage m : listeMessage) {
					nbActu++;
					majMessagerie(p_progress, textArea, imapFolder,
	...
	private static void majMessagerie(JProgressBar p_progress,
			JTextArea textArea, IMAPFolder fldr, MlListeMessage listeMessage,
			int nbActu, MlMessage m) {
		AccesTableMailRecu accesMail = new AccesTableMailRecu();
	...
	private static void releveDossier(MlCompteMail p_compteMail,
		...
		try {
			imapFolder.open(Folder.READ_WRITE);
			int imapcount = imapFolder.getMessageCount();
			int messBaseCount = accesMail.getnbMessageParDossier(p_compteMail
			...
			for (Message m : tabMessageARelever) {
				...
				if (accesMail.verifieAbscenceUID(imapFolder.getUID(m),
						p_idDossier)) {
					...
					Historique.ecrireReleveBal(p_compteMail,
							"Enregistrement du message dans la base");
					accesMail.createNewMessage(messPourBase);
