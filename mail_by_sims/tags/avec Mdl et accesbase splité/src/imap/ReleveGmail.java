...
public class ReleveGmail {
	...
	private void traiteListeDossier(MlCompteMail p_compteMail,
		...
		if (lstSousDossier != null) {
			for (IMAPFolder fldr : lstSousDossier) {
				...
				int idDossier = -1;
				idDossier = verifieConnaissanceDossier(p_compteMail, fldr);
				if (idDossier == (-1)) {// si le sous dossier est
					...
					if (fldr.getFullName().contains("[Gmail]")) {
						...
						accesDossier.createNewDossier(p_compteMail
								.getIdCompte(), 0, fldr.getName(), fldr
								.getFullName());
					} else {
						// sinon, les dossiers créés le seront sous INBOX
						accesDossier.createNewDossier(p_compteMail
								.getIdCompte(), p_compteMail.getIdInbox(), fldr
								.getName(), fldr.getFullName());
					}

					idDossier = accesDossier.getIdDossier(fldr.getName(),
				...
				traiteListeDossier(cptMail, progressBar, progressPJ, props,
						lstFolder, p_store);
	...
	private int verifieConnaissanceDossier(MlCompteMail p_compteMail,
		...
		if (isInbox(fldr)) {
			...
			idDossier = accesDossier.getIdDossier(fldr.getName(), p_compteMail
					.getIdCompte());
		}

		accesDossier.majNomDossierInternet(idDossier, fldr.getFullName());
		return idDossier;
	}
