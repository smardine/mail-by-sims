	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("[releve ]Relever tous les comptes".equals(actionCommand)) {
			...
			thread_SynchroImap t = new thread_SynchroImap(pbReleve,
					pbPieceJointe, text, scrollPane, false, MlListeCompteMail
							.getListeCompte());
			...
			thread_SynchroImap t = new thread_SynchroImap(pbReleve,
					pbPieceJointe, text, scrollPane, true, MlListeCompteMail
							.getListeCompte());
			t.start();
			...
			// lst.add(cpt);
			thread_SynchroImap t = new thread_SynchroImap(pbReleve,
					pbPieceJointe, text, scrollPane, false, cpt);
			t.start();
			...
			// lst.add(cpt);
			thread_SynchroImap t = new thread_SynchroImap(pbReleve,
					pbPieceJointe, text, scrollPane, true, cpt);
			t.start();
		}
		// bd.closeConnexion();
